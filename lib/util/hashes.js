...
module.exports = exports = function (config) {
    ...
    h.findRoot = (opt, results) => {
        ...
        if (results.length === 1)
            return Buffer.from(bs58.decode(results[0].hash));
        else {
            const last = results[results.length -1];
            if (last.path === opt.directory)
                return Buffer.from(bs58.decode(last.hash));
            else
    ...
    h.validate = (hash) => {
        if (typeof hash === 'string') hash = Buffer.from(bs58.decode(hash))
        ...
        if (opt.indexed) {
            results.forEach(r => {
                if ((r.path) === opt.directory) {
                    hash = Buffer.from(bs58.decode(r.hash));
                }
            });
        }
        else {
